...
- [Sparkle](https://sparkle-project.org/) によるアップデート確認（メニューの
  「アップデートを確認…」、1日ごとの自動チェックにも対応）。ad-hoc署名のため、
  自動ダウンロード後のインストールが失敗することがあります。その場合は
  GitHubのリリースページを開く案内を表示するので、そこから手動更新してください
