...
- メニューの「アップデートを確認…」で新バージョンの有無をチェック（GitHub
  Releases APIを参照するだけで、ダウンロードや自動インストールは一切行いません。
  ad-hoc署名アプリは自動インストール時のコード署名検証で失敗しがちなため、
  新バージョンがあればリリースページをブラウザで開く案内にとどめています）
...
```bash
brew tap Blackphi6/vestige
brew install --cask vestige
```

アップデートは `brew upgrade --cask vestige`。
...
起動後、最初にアプリを選択したタイミングで「sfltool」の認証ダイアログが一度
表示されることがあります（ログイン項目/バックグラウンドタスクの登録状況を読む
`sfltool dumpbtm` の実行に、ad-hoc署名アプリからだと認証が必要なため）。この
ダイアログはmacOS標準のAuthorization Servicesが表示するもので、Vestige側から
Touch ID対応を追加することはできません（そのMacにTouch IDセンサーがあれば、
OS側の判断で自動的にオプションが出ます）。読み取り結果は7日間ローカルに
キャッシュされるので、次にまた求められるのは7日後です。
...
エラーが出た場合はそちらを確認してください。Homebrew Cask/Formula のアンインストール
（`brew uninstall`）も Vestige のプロセス権限で実行されるため、同様に失敗することが
あります。

Homebrew Cask/Formula のアンインストールでは、`brew uninstall` の実行後に
Caskroom/Cellar 内の実体が本当に消えたかを自動検証し、消えていなければ1回だけ
自動で再試行します。それでも残っていた場合は「アンインストールが完全に終わって
いません」という専用のエラーを表示します（単なる成功扱いにはしません）。放置すると、
Homebrew はまだインストール済みと認識したままになり、後で `brew install` しても
再インストールされない状態になるため、表示が出たらフルディスクアクセスを許可して
から「削除」をやり直してください。
